...
def _select_area(geo: dict, cur_zoom=None, news_hint: str = "") -> None:
    ...
    recent = [r for r in (st.session_state.get("area_recent") or [])
              if (r.get("lat"), r.get("lon")) != (entry["lat"], entry["lon"])]
    st.session_state["area_recent"] = [entry] + recent[:7]
...
def _build_report(sel: dict) -> str:
    """Assemble a Markdown report from everything loaded for this place."""
    lines = [
        f"# {sel.get('area') or 'Selected place'}",
        "",
        f"- **Coordinates:** {sel['lat']:.5f}, {sel['lon']:.5f}",
    ]
    if sel.get("detail"):
        lines.append(f"- **Address:** {sel['detail']}")
    lines.append("")
    ...
    for cat, d in (st.session_state.get("area_cache") or {}).items():
        ...
        if d.get("extract"):
            lines.append(d["extract"])
        if d.get("condition"):  # weather
            lines.append(f"- Condition: {d.get('condition')}, {d.get('temp')}°C "
                         f"(feels {d.get('feels')}°C), humidity {d.get('humidity')}%")
        if "elevation" in d:  # geography
            if d.get("elevation") is not None:
                lines.append(f"- Elevation: {d['elevation']:.0f} m")
            air = d.get("air") or {}
            if air.get("aqi") is not None:
                lines.append(f"- Air quality: US AQI {air['aqi']} ({air.get('label')})")
            astro = d.get("astro") or {}
            if astro.get("sunrise"):
                lines.append(f"- Sunrise/sunset: {astro.get('sunrise')}/{astro.get('sunset')} "
                             f"({astro.get('timezone')})")
        for it in (d.get("items") or [])[:10]:  # news headlines
            lines.append(f"- [{it.get('title','')}]({it.get('url','')}) "
                         f"— {it.get('source','')} {it.get('date','')}")
        for s in (d.get("sources") or [])[:8]:  # topic sources
            lines.append(f"- [{s.get('title','')}]({s.get('url','')})")
        lines.append("")

    lines.append("---\n_Generated by the Area Explorer._")
    return "\n".join(lines)


def render_area_map(api_key: str, model: str, serper_api_key: str,
                    news_count: int, do_summary: bool,
                    news_time: str = "", out_lang: str = "English") -> None:
    ...
    if recent:
        ...
        for i, r in enumerate(recent[:6]):
            label = (r.get("news_hint") or r.get("area") or "place")[:22]
            if cols[i].button(f"📍 {label}", key=f"recent_{i}", use_container_width=True):
                _select_area(r, news_hint=r.get("news_hint", ""))
                st.rerun()
    ...
    with col_info:
        ...
        with st.spinner(f"Loading {category}…"):
            data = _get_category_data(
                ...
                news_time=news_time, out_lang=out_lang,
            )
        ...
        # Export everything gathered for this place as a Markdown report.
        report = _build_report(sel)
        st.download_button(
            "⬇️ Export report (Markdown)", report,
            file_name=f"{(sel.get('area') or 'place').split(',')[0].strip()}-report.md",
    ...
    sel = st.session_state.get("area_selected")
    if sel:
        _render_area_chat(sel, api_key, model, serper_api_key, out_lang=out_lang)
...
def _get_category_data(category, sel, api_key, model, serper_api_key, news_count, do_summary,
    ...
    if category.endswith("News"):
        ...
        if do_summary and items and api_key.strip():
            summary = summarize_news(Groq(api_key=api_key.strip()), model.strip(),
...
def _render_geography(data, sel):
    ...
    if astro.get("timezone"):
        lines.append(f"**Timezone:** {astro['timezone']}")
    if astro.get("sunrise") and astro.get("sunset"):
        lines.append(f"**Sunrise / sunset:** {astro['sunrise']} / {astro['sunset']} (local)")
    st.markdown("  \n".join(lines))

    air = data.get("air") or {}
    if air.get("aqi") is not None:
        st.markdown(
            f"**🫁 Air quality:** US AQI **{air['aqi']}** ({air.get('label', '—')}) · "
            f"PM2.5 {air.get('pm2_5', '—')} · PM10 {air.get('pm10', '—')} µg/m³"
        )
    st.caption("Elevation, air quality & astro: Open-Meteo · admin data: OpenStreetMap")
...
def _area_chat_reply(client, model: str, place: str, context: str, history: list,
    ...
    system = (
        ...
        f"than guessing. Keep answers accurate and to the point. Reply in {lang}.\n\n"
        f"CONTEXT:\n{context}"
    )
        ...
        with st.spinner("Thinking…"):
            try:
                ...
                reply = _area_chat_reply(
                    ...
                    st.session_state["area_chat"], lang=out_lang,
                )
...
with st.sidebar:
    ...
    if mode == RAG_MODE:
        ...
        _RECENCY = {"Any time": "", "Past day": "d", "Past week": "w", "Past month": "m"}
        news_recency = st.selectbox("News recency", list(_RECENCY), index=2)
        news_time = _RECENCY[news_recency]
        out_lang = st.selectbox(
            "Answer language",
            ["English", "Hindi", "Hinglish", "Marathi", "Tamil", "Telugu", "Bengali", "Gujarati"],
            index=0, help="Language for the AI summaries and the chatbot.",
        )
        ...
        st.divider()
        st.caption(
            "search / tap → news · weather · geography · 16 topics · chat\n\n"
